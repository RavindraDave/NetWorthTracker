...
- **e2e suite is stale (known, deferred)** — `e2e/helpers.ts` waits on `.snapshot-editor`/`.dashboard` classes that no longer exist in src, so the Playwright suite fails wholesale for reasons unrelated to app correctness. The 351-test unit suite is the reliable gate. Also note: this remote container ships chromium-1194 while the pinned Playwright 1.59 wants 1217 (worked around with symlinks locally). Repairing the suite = follow-up task.
